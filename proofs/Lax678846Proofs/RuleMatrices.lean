...
namespace Lax678846Proofs.RuleMatrices

open Lax979537.OrderedStructures Lax979537.FixedPointSyntax
open Lax979537.FixedPointSemantics Lax979537.LeastFixedPoints
open Lax979537Proofs.ParameterizedRules Lax979537Proofs.SyntaxOperations
open Lax979537Proofs.TupleOrder Lax979537Proofs.FormulaMacros
open Lax678846Proofs.OrderedFirstOrder Lax678846Proofs.FirstOrderMacros
...
theorem body_noFixedPoints {σ : Vocabulary} {m k : Nat} (rs : List (Rule σ m k)) :
    NoFixedPoints (Lax979537Proofs.ParameterizedRules.body rs) := by
  simp only [Lax979537Proofs.ParameterizedRules.body, noFixedPoints_anyOf, List.forall_mem_map]
  exact fun r _ => rule_noFixedPoints r

theorem operator_eq {σ : Vocabulary} {k : Nat} (rs : List (Rule σ 0 k)) (A : OrderedStructure σ) :
    StageFormulas.operator (Lax979537Proofs.ParameterizedRules.body rs) A =
      operator rs A Fin.elim0 := by
  funext R
  ext a
  have ha : Fin.append a (Fin.elim0 : Fin 0 → Fin A.size) = a := by simp
  change eval (Lax979537Proofs.ParameterizedRules.body rs) A a
    (Fin.cons R (fun r => Fin.elim0 r)) ↔ _
  rw [← ha]
  simp only [Lax979537Proofs.ParameterizedRules.body, eval_anyOf, exists_map, Rule.eval_formula,
...
theorem eval_queries {σ : Vocabulary} {k : Nat} (qs rs : List (Rule σ 0 k))
    ...
    eval (anyOf (qs.map fun r => Lax979537Proofs.RuleQuery.formula r rs)) A Fin.elim0
      (fun r => Fin.elim0 r) := by
  simp only [queries, eval_anyOf, exists_map, eval_query,
    Lax979537Proofs.RuleQuery.eval_formula, Fin.cons_zero]
  rfl
...
theorem exists_matrix_iff {σ : Vocabulary} {k : Nat} (qs rs : List (Rule σ 0 k))
    ...
    (∃ η : RelationEnv A.size [(k + 1) + k],
      eval (LfpElimination.matrix (Lax979537Proofs.ParameterizedRules.body rs) (queries qs) (k + 1))
        A Fin.elim0 η) ↔
      eval (anyOf (qs.map fun r => Lax979537Proofs.RuleQuery.formula r rs)) A Fin.elim0
        (fun r => Fin.elim0 r) := by
  have hop := operator_eq rs A
  have hm : Monotone (StageFormulas.operator (Lax979537Proofs.ParameterizedRules.body rs) A) :=
    hop.symm ▸ operator_monotone rs A Fin.elim0
  have he := LfpElimination.exists_matrix_iff (Lax979537Proofs.ParameterizedRules.body rs)
...
end Lax678846Proofs.RuleMatrices
