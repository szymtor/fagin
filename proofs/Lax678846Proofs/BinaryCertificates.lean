...
namespace Lax678846Proofs.BinaryCertificates
...
def decode {n k : Nat} (R : Interpretation [k,k] n) : List Bool :=
  select ((Lax979537.StructureEncoding.tuples n k).map (R 0))
    ((Lax979537.StructureEncoding.tuples n k).map (R 1))

def encode (n k : Nat) (c : List Bool) : Interpretation [k,k] n :=
  Fin.cons
    (Decoding.readTable (Lax979537.StructureEncoding.tuples n k)
...
theorem decode_length {n k : Nat} (R : Interpretation [k,k] n) :
  ...
  have h := select_length ((Lax979537.StructureEncoding.tuples n k).map (R 0))
    ((Lax979537.StructureEncoding.tuples n k).map (R 1))
...
theorem decode_encode (n k : Nat) (c : List Bool) (hc : c.length ≤ n ^ k) :
  ...
  have hm := DecoderSoundness.readTable_map (Lax979537.StructureEncoding.tuples n k)
    (List.replicate c.length true ++ List.replicate (n ^ k - c.length) false)
    (DecoderSoundness.tuples_nodup n k) (by simp [StructureEncoding.tuples_length]; omega)
  have hb := DecoderSoundness.readTable_map (Lax979537.StructureEncoding.tuples n k)
    (c ++ List.replicate (n ^ k - c.length) false)
    (DecoderSoundness.tuples_nodup n k) (by simp [StructureEncoding.tuples_length]; omega)
  change select ((Lax979537.StructureEncoding.tuples n k).map
    (Decoding.readTable (Lax979537.StructureEncoding.tuples n k) _))
    ((Lax979537.StructureEncoding.tuples n k).map
      (Decoding.readTable (Lax979537.StructureEncoding.tuples n k) _)) = c
...
end Lax678846Proofs.BinaryCertificates
