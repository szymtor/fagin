...
namespace Lax678846Proofs.StackTag
...
def join (word dst tmp : K) : BitProgram K Aux :=
  .seq (Lax979537Proofs.StackTransfer.transfer word tmp)
...
theorem join_store (word dst tmp : K) (hwd : word ≠ dst) (hwt : word ≠ tmp) (hdt : dst ≠ tmp)
    ...
    Executes (join word dst tmp) s
      ⟨(s.state.1, none), Function.update (Function.update s.stk word []) dst
        (Lax678846.NondeterministicPolynomialTime.encodePair (s.stk word, s.stk dst))⟩
  ...
  have h1 := Lax979537Proofs.StackTransfer.transfer_store word tmp hwt s
  ...
  have ho : (⟨(v.state.1, none), Function.update (Function.update v.stk tmp []) dst
      ...
      ⟨(s.state.1, none), Function.update (Function.update s.stk word []) dst
        (Lax678846.NondeterministicPolynomialTime.encodePair (s.stk word, s.stk dst))⟩ := by
    apply Store.ext
    · rfl
    · funext key
      by_cases hd : key = dst
      · subst key; simp [Lax678846.NondeterministicPolynomialTime.encodePair, tagged]
      · by_cases ht : key = tmp
