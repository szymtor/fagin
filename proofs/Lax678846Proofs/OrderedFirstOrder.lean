...
namespace Lax678846Proofs.OrderedFirstOrder

open Lax678846.FiniteStructures Lax678846.ExistentialSecondOrder
open Lax979537.FixedPointSyntax
...
theorem eval_translate {σ : Vocabulary} {m : Nat} {τ : Vocabulary}
    ...
    (translate φ).eval A (withOrder R) v ↔
      Lax979537.FixedPointSemantics.eval φ A v (fun r a => R r a = true) := by
  induction φ with
  | truth | equal | relation => rfl
  | less x y => simp [translate, FirstOrder.eval, withOrder,
      Lax979537.FixedPointSemantics.eval, Function.comp_def]
  | «variable» r args => rfl
...
end Lax678846Proofs.OrderedFirstOrder
