...
namespace Lax678846Proofs.PreprocessorCertificate
...
theorem program_executes {σ : Vocabulary} {k : Nat} {p : Polynomial Nat}
    ...
    (ht : ∀ r, s.stk (table σ k p r) =
      (Lax979537.StructureEncoding.tuples B.size ((k :: k :: σ).get r)).map (B.relation r))
    (hc : s.stk (certificate σ k p) = []) :
    Executes (program σ k p) s (result B s) (5 * B.size ^ k + 3) := by
  have hm : s.stk (table σ k p 0) =
      (Lax979537.StructureEncoding.tuples B.size k).map ((RelationalVerifier.tables B) 0) := ht 0
  have hd : s.stk (table σ k p 1) =
      (Lax979537.StructureEncoding.tuples B.size k).map ((RelationalVerifier.tables B) 1) := ht 1
...
end Lax678846Proofs.PreprocessorCertificate
