...
namespace Lax678846Proofs.StackCertificateHeader
...
def prepend (domain certificate tmp : K) : BitProgram K Aux :=
  .seq (.atom (.push certificate (fun _ => false)))
    (Lax979537Proofs.StackCopy.copy domain certificate tmp)
...
theorem prepend_executes (domain certificate tmp : K)
    ...
    Executes (prepend domain certificate tmp) s
      ⟨(s.state.1, none), Function.update s.stk certificate
        (Lax979537.StructureEncoding.unary n ++ s.stk certificate)⟩ (7 * n + 5) := by
  ...
  have he := Lax979537Proofs.StackCopy.copy_store domain certificate tmp hdc hdt hct u htmp
  ...
  have hf : (⟨(u.state.1, none), Function.update u.stk certificate
      ...
      ⟨(s.state.1, none), Function.update s.stk certificate
        (Lax979537.StructureEncoding.unary n ++ s.stk certificate)⟩ := by
    apply Store.ext
    · rfl
    · simp [u, Op.apply, Lax979537.StructureEncoding.unary, List.append_assoc]
  rw [hf] at he
  have h := Executes.seq hp he
  convert h using 1 <;> omega

end Lax678846Proofs.StackCertificateHeader
