...
namespace Lax988886Proofs.PreprocessorMachine
...
theorem prepare_encode {σ : Vocabulary} (k : Nat) (p : Polynomial Nat) (B : Structure (k :: k :: σ)) :
  ...
  have h := Decoding.decode_encode (⟨B, Fin.elim0⟩ : Lax751879.OrderedStructures.PointedStructure (k :: k :: σ) 0)
  unfold prepare
  erw [h]
...
theorem compute_executes (σ : Vocabulary) (k : Nat) (p : Polynomial Nat) (xs : List Bool) :
    ∃ d t, d ≤ (computeCost σ k p).eval xs.length ∧
      ...
      t.stk (pair σ k p) = GuardedVerifier.encode Lax988886.NondeterministicPolynomialTime.encodePair
        (prepare σ k p xs) := by
  ...
  | none =>
    ...
    · simp only [computeCost, Polynomial.eval_add, Polynomial.eval_ofNat]
      omega
    · simp [rejected, prepare, hparse, GuardedVerifier.encode, Lax988886.NondeterministicPolynomialTime.encodePair]
  | some B =>
    ...
    ∃ d, d ≤ (cost σ k p).eval xs.length ∧
      Executes (program σ k p) (ioStore (input σ k p) FiniteDecoder.initial xs)
        (ioStore (pair σ k p) FiniteDecoder.initial
          (GuardedVerifier.encode Lax988886.NondeterministicPolynomialTime.encodePair (prepare σ k p xs))) d := by
...
theorem computable (σ : Vocabulary) (k : Nat) (p : Polynomial Nat) :
    Nonempty (Turing.TM2ComputableInPolyTime id
      (GuardedVerifier.encode Lax988886.NondeterministicPolynomialTime.encodePair) (prepare σ k p)) := by
  apply program_polytime (program σ k p) (input σ k p) (pair σ k p) FiniteDecoder.initial
    id (GuardedVerifier.encode Lax988886.NondeterministicPolynomialTime.encodePair) (prepare σ k p) (cost σ k p)
  exact program_executes σ k p

end Lax988886Proofs.PreprocessorMachine
