...
namespace Lax988886Proofs.IterationCertificates
...
theorem stable_after {α : Type} [Fintype α] (F : Set α → Set α)
  ...
  clear hN
  induction d with
  | zero => exact Lax751879Proofs.LeastFixedPoints.finiteConvergence F hF
  | succ d ih =>
      rw [Nat.add_succ, stage, ih]
      exact Lax751879Proofs.LeastFixedPoints.fixedPoint F hF
...
theorem certificate_iff {α : Type} [Fintype α] (F : Set α → Set α)
  ...
  · rintro ⟨H, hH, hR⟩
    rw [← hR, unique hH]
    exact Lax751879Proofs.LeastFixedPoints.finiteConvergence F hF
  · intro hR
    refine ⟨fun t => stage F t.val, ⟨rfl, fun _ => rfl⟩, ?_⟩
    exact (Lax751879Proofs.LeastFixedPoints.finiteConvergence F hF).trans hR.symm

end Lax988886Proofs.IterationCertificates
