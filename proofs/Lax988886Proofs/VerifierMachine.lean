...
namespace Lax988886Proofs.VerifierMachine
...
theorem check_executes (φ : Sentence σ) (w c : List Bool) :
  ...
  | some A =>
    ...
    have hL : (Lax751879.StructureEncoding.unary A.structureValue.size ++ c).length ≤
        (encodePair (w,c)).length := by
      simp only [Lax751879.StructureEncoding.unary, List.length_append, List.length_replicate,
...
end Lax988886Proofs.VerifierMachine
