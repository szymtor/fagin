...
namespace Lax988886Proofs.PreprocessorWord
...
theorem program_executes {σ : Vocabulary} {k : Nat} {p : Polynomial Nat}
  ...
  have he : (⟨(u.state.1, none), Function.update u.stk (word σ k p)
      (Lax751879.StructureEncoding.unary B.size ++ ExpandedEncoding.payload (RelationalVerifier.base B))⟩ :
  ...
  have hlen : (encode (RelationalVerifier.base B)).length =
    ...
    simp [ExpandedEncoding.encode_eq, Lax751879.StructureEncoding.unary, RelationalVerifier.base]
    omega
  simp only [baseTables, List.length_map, List.length_finRange]
  omega

end Lax988886Proofs.PreprocessorWord
