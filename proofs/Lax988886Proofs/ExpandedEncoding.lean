...
namespace Lax988886Proofs.ExpandedEncoding
...
theorem encode_eq {σ : Vocabulary} (A : Structure σ) :
    encode A = Lax751879.StructureEncoding.unary A.size ++ payload A := by
  simp [encode, Lax751879.StructureEncoding.encode, payload]
...
theorem encode_length {σ : Vocabulary} (A : Structure σ) :
  ...
  rw [show encode A = Lax751879.StructureEncoding.encode ⟨A, Fin.elim0⟩ from rfl,
...
theorem payload_expand {σ : Vocabulary} {k : Nat} (A : Structure σ)
    ...
    payload (RelationalVerifier.expand A R) =
      (Lax751879.StructureEncoding.tuples A.size k).map (R 0) ++
      (Lax751879.StructureEncoding.tuples A.size k).map (R 1) ++ payload A := by
...
theorem encode_expand {σ : Vocabulary} {k : Nat} (A : Structure σ)
    ...
    encode (RelationalVerifier.expand A R) =
      Lax751879.StructureEncoding.unary A.size ++
      (Lax751879.StructureEncoding.tuples A.size k).map (R 0) ++
      (Lax751879.StructureEncoding.tuples A.size k).map (R 1) ++ payload A := by
  rw [encode_eq, payload_expand]
  simp [RelationalVerifier.expand, List.append_assoc]

end Lax988886Proofs.ExpandedEncoding
