...
namespace Lax988886Proofs.StackKeepOutput
...
def clean (keys : List K) (out : K) (initial : Aux × Option Bool) : BitProgram K Aux :=
  .seq (Lax751879Proofs.StackOutput.clearPorts (keys.filter (fun k => decide (k ≠ out))))
...
theorem clean_executes (keys : List K) (hall : ∀ key, key ∈ keys) (out : K)
  ...
  obtain ⟨c, hc, hp⟩ := Lax751879Proofs.StackOutput.clearPorts_executes ks s M hs
  have hr := Executes.atom (.load (fun _ : Aux × Option Bool => initial))
    (Lax751879Proofs.StackOutput.cleared ks s)
  have he : Op.apply (.load (fun _ : Aux × Option Bool => initial))
      (Lax751879Proofs.StackOutput.cleared ks s) = ioStore out initial (s.stk out) := by
    apply Store.ext
    · rfl
    · funext key
      by_cases hk : key = out
      · subst key; simp [Op.apply, Lax751879Proofs.StackOutput.cleared, ks, ioStore]
      · simp [Op.apply, Lax751879Proofs.StackOutput.cleared, ks, ioStore, hk, hall key]
  rw [he] at hr
...
end Lax988886Proofs.StackKeepOutput
